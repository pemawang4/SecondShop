<?php include 'includes/header.php'; 
    ...
    if(isset($_POST['update'])){
        $updateSubCat = $cat->updateSubCat($_POST);
        if($updateSubCat = true){
            echo "<div class='alert alert-success text-center'>Category updated.</div>";
        }else{
    ...
    if(isset($_GET['catId']) && isset($_GET['catId']) != ''){
        $catId = $_GET['catId'];
        $query = "SELECT * FROM subCategories WHERE subCatId = $catId";
        $row = $db->select($query);
        $cat = $row->fetch_assoc();
    }
...
<div class="container">
    <div class="row">
        ...
        <a href="subCat.php" class="btn btn-secondary margin">Go Back</a>
            <div class="card">
                <div class="card-header">
                    <h4>Category</h4>
                </div>  
                <div class="card-body">
                    <form action="" method="POST">
                        <div class="form-group">
                            <input type="hidden" name="catId" value="<?= $cat['subCatId']; ?>">
